...
#define EXAMPLE_DEVICE_NAME                       "Nowa"
...
void esp_get_notification_attributes(int idx, uint8_t *notificationUID, uint8_t num_attr, esp_noti_attr_list_t *p_attr)
{
    ...
    esp_ble_gattc_write_char( gl_profile_tab[idx].gattc_if,
                              ...
                              gl_profile_tab[idx].contol_point_handle,
                              index,
                              cmd,
...
static void periodic_timer_callback(void* arg)
{
    ...
    ESP_LOGV(BLE_ANCS_TAG, "periodic_timer_callback");
...
static void gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    ESP_LOGV(BLE_ANCS_TAG, "GATT_EVT (PRF %u), event %d", idx, event);

    switch (event) {
    case ESP_GATTC_REG_EVT:
        ESP_LOGI(BLE_ANCS_TAG, "REG_EVT: AppId=%u GattcIf=%u", param->reg.app_id, gattc_if);
    ...
    case ESP_GATTC_OPEN_EVT:
        ...
        ESP_LOGI(BLE_ANCS_TAG, "ESP_GATTC_OPEN_EVT conn_id=%u", param->open.conn_id);
    ...
    case ESP_GATTC_SEARCH_RES_EVT: {
        // ESP_LOGI(BLE_ANCS_TAG, "ESP_GATTC_SEARCH_RES_EVT len=%u", param->search_res.srvc_id.uuid.len);
        // if (param->search_res.srvc_id.uuid.len == ESP_UUID_LEN_16) {
        //     ESP_LOGI(BLE_ANCS_TAG, "    UUID=%04X", param->search_res.srvc_id.uuid.uuid.uuid16);
        // } else if (param->search_res.srvc_id.uuid.len == ESP_UUID_LEN_128) {
        //     ESP_LOGI(BLE_ANCS_TAG, "    UUID=%08X-%04X-%04X-%04X-%04X%04X%04X",
        ...
        //     );
        // }
        // ESP_LOGI(BLE_ANCS_TAG, "    StartHandle=%04X EndHandle=%04X", param->search_res.start_handle, param->search_res.end_handle);
        if (param->search_res.srvc_id.uuid.len == ESP_UUID_LEN_128) {
            gl_profile_tab[idx].service_start_handle = param->search_res.start_handle;
            gl_profile_tab[idx].service_end_handle = param->search_res.end_handle;
            gl_profile_tab[idx].service_found = true;
        }
        break;
    }
    case ESP_GATTC_SEARCH_CMPL_EVT:
        ...
        // }
        // ESP_LOGI(BLE_ANCS_TAG, "    esp_ble_gattc_get_attr_count=%u", count);
        // esp_gattc_char_elem_t char_elem_result[10] = {0};
        ...
        // }
        // ESP_LOGI(BLE_ANCS_TAG, "    esp_ble_gattc_get_all_char=%u", count);
        // ESP_LOGI(BLE_ANCS_TAG, "    Handle0=%04X UUID=%04X", char_elem_result[0].char_handle, char_elem_result[0].uuid.uuid.uuid16);
        // ESP_LOGI(BLE_ANCS_TAG, "    Handle1=%04X UUID=%04X", char_elem_result[1].char_handle, char_elem_result[1].uuid.uuid.uuid16);

        // // Name: Handle 0x0003 UUID 0x2A00
        ...
        // );
        // if (ret_status != ESP_GATT_OK) {
        //     ESP_LOGE(BLE_ANCS_TAG, "esp_ble_gattc_read_char error, %d", __LINE__);
        // }

        if (gl_profile_tab[idx].service_found) {
            uint16_t count  = 0;
            uint16_t offset = 0;
            esp_gatt_status_t ret_status = esp_ble_gattc_get_attr_count(gattc_if,
                                                                        gl_profile_tab[idx].conn_id,
                                                                        ESP_GATT_DB_CHARACTERISTIC,
                                                                        gl_profile_tab[idx].service_start_handle,
                                                                        gl_profile_tab[idx].service_end_handle,
                                                                        INVALID_HANDLE,
                                                                        &count);
            if (ret_status != ESP_GATT_OK) {
                ESP_LOGE(BLE_ANCS_TAG, "esp_ble_gattc_get_attr_count error, %d", __LINE__);
            ...
            if (count > 0) {
                gl_profile_tab[idx].char_elem_result = (esp_gattc_char_elem_t *)malloc(sizeof(esp_gattc_char_elem_t) * count);
                memset(gl_profile_tab[idx].char_elem_result, 0xff, sizeof(esp_gattc_char_elem_t) * count);
                if (!gl_profile_tab[idx].char_elem_result) {
                    ...
                    ret_status = esp_ble_gattc_get_all_char(gattc_if,
                                                            ...
                                                            gl_profile_tab[idx].service_start_handle,
                                                            gl_profile_tab[idx].service_end_handle,
                                                            gl_profile_tab[idx].char_elem_result,
                                                            &count,
                                                            offset);
                    if (ret_status != ESP_GATT_OK) {
                        ESP_LOGE(BLE_ANCS_TAG, "esp_ble_gattc_get_all_char error, %d", __LINE__);
                    ...
                    if (count > 0) {
                        ...
                        for (int i = 0; i < count; i ++) {
                            if (gl_profile_tab[idx].char_elem_result[i].uuid.len == ESP_UUID_LEN_128) {
                                if (gl_profile_tab[idx].char_elem_result[i].properties & ESP_GATT_CHAR_PROP_BIT_NOTIFY
                                    && memcmp(gl_profile_tab[idx].char_elem_result[i].uuid.uuid.uuid128, notification_source, 16) == 0) {
                                    gl_profile_tab[idx].notification_source_handle = gl_profile_tab[idx].char_elem_result[i].char_handle;
                                    esp_ble_gattc_register_for_notify (gattc_if,
                                                                       gl_profile_tab[idx].remote_bda,
                                                                       gl_profile_tab[idx].char_elem_result[i].char_handle);
                                    ESP_LOGI(BLE_ANCS_TAG, "Found Apple notification source char");

                                } else if (gl_profile_tab[idx].char_elem_result[i].properties & ESP_GATT_CHAR_PROP_BIT_NOTIFY
                                            && memcmp(gl_profile_tab[idx].char_elem_result[i].uuid.uuid.uuid128, data_source, 16) == 0) {
                                    gl_profile_tab[idx].data_source_handle = gl_profile_tab[idx].char_elem_result[i].char_handle;
                                    esp_ble_gattc_register_for_notify (gattc_if,
                                                                       gl_profile_tab[idx].remote_bda,
                                                                       gl_profile_tab[idx].char_elem_result[i].char_handle);
                                    ESP_LOGI(BLE_ANCS_TAG, "Found Apple data source char");

                                } else if (gl_profile_tab[idx].char_elem_result[i].properties & ESP_GATT_CHAR_PROP_BIT_WRITE
                                            && memcmp(gl_profile_tab[idx].char_elem_result[i].uuid.uuid.uuid128, control_point, 16) == 0) {
                                    gl_profile_tab[idx].contol_point_handle = gl_profile_tab[idx].char_elem_result[i].char_handle;
                                    ESP_LOGI(BLE_ANCS_TAG, "Found Apple control point char");
                ...
                free(gl_profile_tab[idx].char_elem_result);
            }
        } else {
    ...
    case ESP_GATTC_READ_CHAR_EVT:
        ESP_LOGI(BLE_ANCS_TAG, "Status=%u", param->read.status);
    ...
    case ESP_GATTC_REG_FOR_NOTIFY_EVT: {
        ...
        if (count > 0) {
            gl_profile_tab[idx].descr_elem_result = malloc(sizeof(esp_gattc_descr_elem_t) * count);
            if (!gl_profile_tab[idx].descr_elem_result) {
                ESP_LOGE(BLE_ANCS_TAG, "malloc error, gattc no mem");
            } else {
                ret_status = esp_ble_gattc_get_all_descr(gattc_if,
                                                         gl_profile_tab[idx].conn_id,
                                                         param->reg_for_notify.handle,
                                                         gl_profile_tab[idx].descr_elem_result,
                                                         &count,
                                                         offset);
                if (ret_status != ESP_GATT_OK) {
                ...
                for (int i = 0; i < count; ++ i) {
                    if (gl_profile_tab[idx].descr_elem_result[i].uuid.len == ESP_UUID_LEN_16 && gl_profile_tab[idx].descr_elem_result[i].uuid.uuid.uuid16 == ESP_GATT_UUID_CHAR_CLIENT_CONFIG) {
                        esp_ble_gattc_write_char_descr (gattc_if,
                                                        gl_profile_tab[idx].conn_id,
                                                        gl_profile_tab[idx].descr_elem_result[i].handle,
                                                        sizeof(notify_en),
                                                        (uint8_t *)&notify_en,
                                                        ESP_GATT_WRITE_TYPE_RSP,
                                                        ESP_GATT_AUTH_REQ_NONE);
            ...
            free(gl_profile_tab[idx].descr_elem_result);
        }
        break;
    }
    case ESP_GATTC_NOTIFY_EVT:
        ESP_LOGV(BLE_ANCS_TAG, "Rx notification: %u bytes", param->notify.value_len);
        esp_log_buffer_hex(BLE_ANCS_TAG, param->notify.value, param->notify.value_len);
        if (param->notify.handle == gl_profile_tab[idx].notification_source_handle) {
        ...
        } else if (param->notify.handle == gl_profile_tab[idx].data_source_handle) {
            ...
            if (param->notify.value_len == (gl_profile_tab[idx].MTU_size - 3)) {
                ...
                ESP_LOGV(BLE_ANCS_TAG, "Starting timer");
    ...
    case ESP_GATTC_DISCONNECT_EVT:
        ...
        ESP_LOGI(BLE_ANCS_TAG, "Disconnecting this profile");
        gl_profile_tab[idx].service_found = false;
    ...
    case ESP_GATTC_CONNECT_EVT:
        ...
        if (idx == PROFILE_A_APP_ID) {
            ...
            ESP_LOGI(BLE_ANCS_TAG, "Advertising restart: %x", ret);
        }
        ...
        ESP_LOGI(BLE_ANCS_TAG, "Binding to this profile");
...
esp_err_t ancs_init (void)
{
    ...
    esp_log_level_set(BLE_ANCS_TAG, ESP_LOG_VERBOSE);
